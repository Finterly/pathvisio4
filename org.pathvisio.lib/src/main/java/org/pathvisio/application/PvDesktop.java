...
public class PvDesktop
	...
	public PvDesktop(SwingEngine swingEngine, BundleContext context) {
		...
		visualizationManager = new VisualizationManager(swingEngine.getEngine(), gexManager);
	...
	/**
	 * Load the Gex cache for the current pathway. Only starts loading when an
	 * expression dataset is available and a pathway is open.
	 */
	public void loadGexCache() {
		final CachedData gex = gexManager.getCachedData();
		final PathwayModel p = swingEngine.getEngine().getActivePathway();
		if (p != null && gex != null) {
			try {
				gex.clearCache();
				gex.setMapper(swingEngine.getGdbManager().getCurrentGdb());
				gex.preSeed(p.getDataNodeXrefs());
				gex.preSeed(p.getStateXrefs()); // TODO
				gex.preSeed(p.getInteractionXrefs()); // TODO
				gex.preSeed(p.getGroupXrefs()); // TODO
