...
public class PathVisioMain {
	...
	/**
	 * The smoke-test option is for automated testing purposes. When set, PathVisio
	 * just tries loading plugins, and quits with exit code 0 on success or non-zero
	 * on error.
	 */
	private static boolean isSmokeTest = false;

	/**
	 * PathVisio main method.
	 * 
	...
	/**
	 * The following bundles below must activate without exception. If there is an
	 * exception while activating one of the bundles below, an error message will be
	 * reported directly to the user, and PathVisio shuts down. Exceptions in other
	 * bundles are merely logged.
	 */
	public static final List<String> mustActivate = Arrays.asList(new String[] { "derby", "org.w3c.dom.events",
			"org.bridgedb.rdb", "org.pathvisio.core", "org.pathvisio.gui", "org.pathvisio.desktop",
			"org.pathvisio.visualization", "org.pathvisio.gex", "org.pathvisio.statistics" });
	...
	private static final String[][] frameworkProperties = {
			/*
			 * org.osgi.framework.bootdelegation=*, this means that all system classes are
			 * available to any bundle.
			 * 
			 * when running under felix, the following is necessary for batik to work
			 * properly, otherwise it will throw NoClassDefFoundError's left and right.
			 */
...
//        {"org.osgi.framework.storage.clean", "none"},
			...
			/*
			 * following property is necessary for Felix: to prevent complaints about
			 * missing requirements ee=JSE2-1.2 on the javax.xml bundle.
			 */
			{ "org.osgi.framework.executionenvironment",
					"ee-1.6=JavaSE-1.6,J2SE-1.5,J2SE-1.4,J2SE-1.3,J2SE-1.2,JRE-1.1,JRE-1.0,OSGi/Minimum-1.2,OSGi/Minimum-1.1,OSGi/Minimum-1.0" }
	};

	/**
	 * Determine which FrameWork factory class is available by reading a property in
	 * the jar file. This makes it easy to switch between felix, equinox or another
	 * OSGi implementation. All you have to do is include either felix.jar or
	 * org.eclipse.osgi.jar in the classpath.
	 * 
	 * @throws IOException
	 * @return factoryClass
	 */
	private String getFactoryClass() throws IOException {
		BufferedReader factoryReader = new BufferedReader(new InputStreamReader(getClass().getClassLoader()
	...
	/**
	 * @return launchProperties the map of launch properties.
	 */
	...
	private void startBundles(BundleContext context, Map<Bundle, String> bundles) throws BundleException {
		...
		if (mustActivateLeft.size() > 0) {
			...
			for (String s : mustActivateLeft)
				missing.append(" " + s);
			JOptionPane.showMessageDialog(null, "Fatal: some essential bundles were missing: " + missing);
			System.exit(1);
		}

		/* if we're doing a smoke test, and we got to this point, we've completed successfully. */
	...
	/**
	 * Start a single bundle, record any exceptions and update the mustActivateLeft
	 * set
	 * 
	 * @param b                the bundle.
	 * @param mustActivateLeft the set.
	 */
	public void startBundle(Bundle b, Set<String> mustActivateLeft) {
		...
		try {
			...
			System.out.println(
					"Core Bundle " + b.getBundleId() + " with location " + b.getLocation() + " failed to start.");
			if (mustActivateLeft.contains(symbolicName)) {
	...
	/**
	 * Returns bundle cache file.
	 * 
	 * @return file the bundle cache file.
	 */
	...
	/**
	 * Returns true if string is argument, false otherwise.
	 * 
	 * @param string the string.
	 * @return true if string is argument, false otherwise.
	 */
	private static boolean isArgument(String string) {
		if (string.equals("-p") || string.equals("-v") || string.equals("-h") || string.equals("-d")
				|| string.equals("--smoketest")) {
			return true;
		}
		return false;
	}
	...
	private static void printHelp() {
		...
		/*
		 * NOTE: the --smoketest option is not documented on purpose It's not for use by
		 * end-users.
		 */
		);
	}
