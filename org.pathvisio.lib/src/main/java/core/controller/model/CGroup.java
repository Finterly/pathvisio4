 ...
 */
public class CGroup extends Controller implements LinkProvider, VElementMouseListener {
	...
	public CGroup(CPathwayModel canvas, Group pe) {
		super(canvas);
		// pe.addListener(this); 
		gdata = pe;
	...
	protected Map<String, String> getIdRefPairs() {
		...
		for (VElement vpe : canvas.getDrawingObjects()) {
			if (vpe instanceof Controller && vpe instanceof VGroup) {
				Group pe = ((Controller) vpe).getPathwayElement();
	...
	protected boolean vContains(Point2D point) {
		...
		for (VElement vpe : canvas.getDrawingObjects()) {
			if (vpe instanceof Controller && !(vpe instanceof VGroup) && vpe.vContains(point)) {
				return false;
	 ...
	 */
	public List<Controller> getGroupGraphics() {
		List<Controller> gg = new ArrayList<Controller>();
		// return true if group object is referenced by selection
		for (VElement vpe : canvas.getDrawingObjects()) {
			if (vpe instanceof Controller && vpe != this) {
				Controller vpeg = (Controller) vpe;
	...
	@Override
	public void select() {
		for (Controller g : getGroupGraphics()) {
			g.select();
		}
		super.select();
	}
