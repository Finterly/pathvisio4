...
/**
 * This {@link Controller} class represents the view of a {@link Shape},
 * {@link Label}, or {@link DataNode} PathwayElement. Rotation is implemented
 * with 8 handles placed in a (rotated) rectangle around the shape and a
 * rotation handle. //TODO
 * 
 * @author unknown, finterly
 */
public abstract class ControllerShape extends ControllerShapedElement implements LinkProvider, Adjustable {

	protected org.pathvisio.model.Shape gdata = null; // TODO Only shape has rotation for libGPML
	 ...
	 */
	public ControllerShape(CPathwayModel canvas) {
		super(canvas);
	}
	...
	protected void createHandles() {
		if (gdata.getShapeStyleProperty().getShapeType() != null && !gdata.getShapeType().isResizeable()
		...
		} else if (gdata.getShapeStyleProperty().getShapeType() != null && !gdata.getShapeType().isResizeable()
				&& gdata.getShapeType().isRotatable()) {
			handleR = new Handle(Handle.Freedom.ROTATION, this, this);
			handleR.setAngle(1);
			handles = new Handle[] { handleR };
		} else if (this instanceof CState) {
			...
			if (this instanceof VDataNode || this instanceof CLabel || !gdata.getShapeType().isRotatable()) {
	...
	protected void setHandleLocation() {
		...
		if ((gdata.getShapeStyleProperty().getShapeType() == null || gdata.getShapeType().isRotatable()) && (handleR != null)) {
